...
class WanAttnProcessor2_0:
    ...
    def __call__(
        ...
        if attn.add_k_proj is not None: 
            ...
            # print(encoder_hidden_states_img.size(), encoder_hidden_states.size())
...
class TagAttention(nn.Module): 
    def __init__(
        ...
        self.k = nn.Linear(caption_dim, dim, bias=qkv_bias)
        self.v = nn.Linear(caption_dim, dim, bias=qkv_bias) 
    ...
    def forward(self, 
        ...
        B, N_img, C_img = x_img.shape 
        B, N_tag, C_tag = x_tag.shape
        # qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        # q, k, v = qkv.unbind(0) 
        # print(x_img.size(), x_tag.size())
        q = self.q(x_img).reshape(B, N_img, self.num_heads, self.head_dim).permute(0, 2, 1, 3) 
        k = self.k(x_tag).reshape(B, N_tag, self.num_heads, self.head_dim).permute(0, 2, 1, 3) 
        v = self.v(x_tag).reshape(B, N_tag, self.num_heads, self.head_dim).permute(0, 2, 1, 3) 
        ...
        x = x.transpose(1, 2).reshape(B, N_img, C_img)
        x = self.proj(x)
        x = self.proj_drop(x) 
...
class A2Model(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    r"""
    A Transformer model for video-like data used in the Wan model.
